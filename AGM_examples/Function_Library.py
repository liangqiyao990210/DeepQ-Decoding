...
MaxPooling2D, ZeroPadding2D, GlobalAveragePooling2D, Flatten, Lambda, \
Cropping2D, Activation, Input, merge, Concatenate
...
def build_noisy_nn(cc_layers,ff_layers, input_shape, num_actions):
    """"

    This function builds a noisy convolutional neural network:
!
    :param: cc_layers: [[num_filters, kernel_size,strides],...]
    :param: ff_layers: [[neurons, output_dropout_rate],...]
    :param: input_shape: The shape of the input - i.e. num channels and image height,width
    ...
    for j in range(len(ff_layers)):
        model.add(NoisyDense(ff_layers[j][0]))
        model.add(Activation('relu'))
        model.add(Dropout(rate=ff_layers[j][1]))
...
class NoisyDense(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.mu_interval = 1.0 / np.sqrt(self.p)
        self.mu_init = tf.initializers.random_uniform(minval = -self.mu_interval,
                                                      maxval =  self.mu_interval)

        self.si_0 = 0.5
        self.si_init_const = self.si_0 / np.sqrt(self.p)
        self.si_init = tf.initializers.constant(self.si_init_const)

        self.w_mu = self.add_weight(shape=(input_shape[1], self.units),
                                    initializer=self.mu_init, trainable=True)
        self.w_si = self.add_weight(shape=(input_shape[1], self.units),
                                    initializer=self.si_init, trainable=True)

        self.b_mu = self.add_weight(shape=(self.units,),
                                    initializer=self.mu_init, trainable=True)
        self.b_si = self.add_weight(shape=(self.units,),
                                    initializer=self.si_init, trainable=True)

        super(NoisyDense, self).build(input_shape) # not sure if needed

    def call(self, inputs):
        self.sample_noise()

        self.w = tf.math.add(self.w_mu, tf.math.multiply(self.w_si, self.w_eps))
        self.b = tf.math.add(self.b_mu, tf.math.multiply(self.b_si, self.b_eps))

        return tf.math.add(tf.linalg.matmul(inputs, self.w), self.b)

    def sample_noise(self):
        self.eps_i = tf.random.normal([self.p, 1])
        self.eps_j = tf.random.normal([1, self.units])
        self.w_eps = self.f(self.eps_i) * self.f(self.eps_j)
        self.b_eps = self.f(self.eps_j)
