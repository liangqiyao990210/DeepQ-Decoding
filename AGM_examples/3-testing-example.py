...
# production setting. In this notebook we will demonstrate how to perform both
# of these tasks.
#
# ##### 3a) Evaluating a Trained Decoder
#
# Given a trained decoder we would of course like to benchmark the decoder to
# evaluate how well it performs. This procedure is very similar to training the
# decoder, in that we run multiple decoding episodes in which the agent
...
# parameters in time. As we will see benchmarking an agent is made easy by use
# of the DQNAgent class "test" method.
...
fixed_configs_path = os.path.join(os.getcwd(),
    "../trained_models/d5_x/fixed_config.p")
variable_configs_path = os.path.join(os.getcwd(),
    "../trained_models/d5_x/0.007/variable_config_77.p")
model_weights_path = os.path.join(os.getcwd(),
    "../trained_models/d5_x/0.007/final_dqn_weights.h5f")

static_decoder_path = os.path.join(os.getcwd(),
    "referee_decoders/nn_d5_X_p5")
...
# Now we build a model and instantiate an agent with all the parameters of
# the pre-trained agent. Notice that we insist on a greedy policy!
...
# At this stage the agent has random weights, and so we load in the weights of
...
#
# It is important to note that the reported episode length is the number of
# _non-trivial_ syndrome volumes that the agent received, as these are the
# steps during which a decision needs to be taken on the part of the agent. The
# qubit lifetime, whose rolling average is reported, is the total number of
...
# Here we see that on average, over 1001 test episodes, the qubit survives for
# 329 syndrome measurements on average, which is better than the average
...
#
# In addition to benchmarking a decoder via the agent test method, we would
# like to demonstrate how to use the decoder in practice, given a faulty
# syndrome volume. In principle all the information on how to do this is
# contained within the environments and test method, but to aid in applying
# these decoders quickly and easily in practice we make everything explicit
# here:
#
...
for j in range(d):
    ...
    current_true_syndrome = generate_surface_code_syndrome_NoFT_efficient(
        hidden_state, qubits)
    current_faulty_syndrome = generate_faulty_syndrome(current_true_syndrome,
        p_meas)
...
# And now we would like to decode and obtain the suggested corrections. To do
# this, we begin by padding the faulty syndromes as required and by
# concatenating the obtained volume with an action history slice, in which all
# the actions are initially zero:
...
while still_decoding:
    ...
    if action not in corrections and action != env.identity_index:
        ...
        # Update the input state to the agent to indicate the correction it
        # would have made
...
# And now we can view the suggested corrections, which in this case was a
# single correct suggestion:
...
# interested in how long the qubit survives for, and an optimal strategy for
# achieving long qubit lifetimes may not be to attempt to fully decode into the
# ground state after each syndrome volume!
