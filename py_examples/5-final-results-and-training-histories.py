...
#
# As we have discussed, during the course of the training procedure the results
# of all trained agents in "test_mode" are written out and stored, as well as
# the results from the best agent at each error rate. However, in order to ease
# computational time during training, each agent was only benchmarked for 100
# episodes.
#
# Here we present the full results and training histories for each best
# performing agent, as selected by the preliminary benchmarking during
# training. In particular, these full results were obtained by testing each
# best performing trained agent, at each error rate, for the number of episodes
# that guaranteed at least 10^6 syndromes were seen by the agent. All the
# trained agents from which these results were obtained, along with the fully
# detailed results (i.e. episode length of every single tested episode at each
# error rate for each agent), can be found in the "trained_models" directory of
# the repo. In addition to these evaluation results, we also provide the
# learning curves for all best performing agents.
#
# ##### 5a) Results
#
# We begin by showing the results of full evaluations. In particular, we start
# by importing all necessary packages:
...
variable_keys = ["exploration_fraction", "max_eps", "final_eps",
    "learning_rate", "target_network_update_freq"]
...
# And now we can extract and format the results, both from the bitflip and
# depolarising noise simulations:
...
for direct in os.listdir(x_folder):
    if "fixed_config" not in direct:
        ...
        x_configs_dict[direct] = config_dict_to_string(pickle.load(
            open(path_to_config, "rb" )),variable_keys)
...
for direct in os.listdir(dp_folder):
    if "fixed_config" not in direct:
        ...
        for f in os.listdir(err_direct):
            if "variable_config" in f:
                path_to_config = os.path.join(err_direct,f)
        dp_configs_dict[direct] = config_dict_to_string(pickle.load(
            open(path_to_config, "rb" )),variable_keys)
...
for key in ordered_keys:
    if key in ordered_x_results.keys():
        ...
        if( key != "0.015" ):
            # ax.plot(p_phys[:len(ordered_x_results[key])],ordered_x_results[key],
            # marker=markers[count], markersize=6, markeredgecolor='k',
            # markeredgewidth=0.5, label="$%s$"%key)
            ax[0,0].plot(p_phys[:len(ordered_x_results[key])],ordered_x_results[key],
                marker=markers[count], markersize=6, label="$%s$"%key)
        else:
            ax[0,0].plot(p_phys[:len(ordered_x_results[key])],ordered_x_results[key],
                marker=markers[count], markersize=7, markeredgecolor='k',
                label="$%s$"%key)
        count += 1

ax[0,0].plot(p_phys[:max_l],bench[:max_l],"k")#,label="Single \n Faulty Qubit")
ax[0,0].text(0.0015, 340, "Single Faulty Qubit", rotation=-18, fontsize=10)
ax[0,0].text(0.57, 0.92, "(a) Bitflip Noise - All Agents",
...
ax[0,0].legend(fancybox=True, ncol=2, title="Agent trained at",
    bbox_to_anchor=(0.003, 0.8,1., .102),fontsize=10)
# ax[0,0].legend(fancybox=True, ncol=2, title="Agent trained at",
# bbox_to_anchor=(0.003, 0.33, 1., .102))
...
ax[0,1].text(0.38, 0.92, "(c) Bitflip Noise - Best Performing Agents",
    fontsize=10,transform=ax[0,1].transAxes)
...
for key in ordered_keys:
    if key in ordered_dp_results.keys():
        ...
        if( key != "0.011" ):
            ...
            # marker=markers[count], markersize=6, markeredgecolor='k',
            # markeredgewidth=0.5, label="$%s$"%key)
            ax[1,0].plot(p_phys[:len(ordered_dp_results[key])],ordered_dp_results[key],
                marker=markers[count], markersize=6, label="$%s$"%key)
        else:
            ax[1,0].plot(p_phys[:len(ordered_dp_results[key])],ordered_dp_results[key],
                marker=markers[count], markersize=5, markeredgecolor='k',
                label="$%s$"%key)
        count += 1

ax[1,0].plot(p_phys[:max_l],bench[:max_l],"k")#,label="Single \n Faulty Qubit")
ax[1,0].text(0.0015, 340, "Single Faulty Qubit", rotation=-18, fontsize=10)
ax[1,0].text(0.478, 0.92, "(b) Depolarising Noise - All Agents",
    fontsize=10,transform=ax[1,0].transAxes)
...
ax[1,0].legend(fancybox=True, ncol=2, title="Agent trained at",
    fontsize=10,bbox_to_anchor=(0.002, 0.8, 1., .102))
...
ax[1,1].plot(p_phys[:len(best_results)], best_results, '-o',
    label="Decoded Surface Code")
...
ax[1,1].text(0.29, 0.92, "(d) Depolarising Noise - Best Performing Agents",
    fontsize=10,transform=ax[1,1].transAxes)
...
fig.tight_layout()
fig.savefig("./plots/final_results.png", bbox_inches='tight')


# As discussed in the associated paper, from the above figures one can see that
# for bitflip (depolarising) noise, as long as the error rate is below 0.013
# (0.011), there exists a decoding agent for which the average lifetime of a
# decoded logical qubit is longer than the average lifetime of a single faulty
# qubit.
#
# ##### 5a) Training Histories
#
# It is also of interest to examine the learning curves of all best performing
# agents. As such, we display these below, again for both bitflip and
# depolarising noise. For each best performing agent, we also provide the
# associated hyperparameters at which that agent was obtained, given as a list
# [num_exploration_steps, initial_epsilon, final_epsilon, learning_rate,
...
plt.tight_layout()
plt.savefig("./plots/bitflip_training.png")
# plt.show()
...
plt.tight_layout()
plt.savefig("./plots/depolarising_training.png")
# plt.show()
