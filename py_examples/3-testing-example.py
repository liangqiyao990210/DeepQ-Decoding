...
# Now we build a model and instantiate an agent with all the parameters of the
# pre-trained agent. Notice that we insist on a greedy policy!
...
for j in range(d):
    ...
    current_true_syndrome = generate_surface_code_syndrome_NoFT_efficient(hidden_state,
        qubits)
